...
async function handleAction(category) {
    ...
    try {
        ...
        if (selection.length === 0 && state.dataCache.length > 0) {
            resultsDiv.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:40px;"><button onclick="resetList('${category}')" class="category-btn active" style="margin: 0 auto; width: auto; padding: 10px 20px;">🔄 Start Over</button></div>`;
        } else {
...
function renderCard(item, category) {
    ...
    card.querySelector('p').textContent = desc || "Tap the link below for more details!";
    card.querySelector('.category-tag').textContent = type || category;
    
    const distTag = card.querySelector('.dist-tag');
    if (distValue && category !== 'music') distTag.textContent = distValue; else distTag.remove();
    
    const footer = card.querySelector('.card-footer');
    if (mapsUrl && category !== 'music') {
        const link = document.createElement('a');
        link.href = mapsUrl; link.target = "_blank"; link.rel = "noopener noreferrer";
        link.className = "btn-link"; link.textContent = "📍 Open Google Maps ↗";
        footer.appendChild(link);
    }
    if (musicUrl && category === 'music') {
        const link = document.createElement('a');
        link.href = musicUrl; link.target = "_blank"; link.rel = "noopener noreferrer";
        link.className = "btn-link"; link.textContent = "🎵 Open Spotify ↗";
        footer.appendChild(link);
    }
    return card;
}

function resetList(cat) { state.pointers[cat] = 0; handleAction(cat); }
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Always show overlay on every visit/refresh
